...
let iconCandidates = [
    resourcesURL.appendingPathComponent("StashbarIconDark.png"),
    resourcesURL.appendingPathComponent("StashbarIcon.png"),
    resourcesURL.appendingPathComponent("FileStackIconDark.png"),
...
guard let iconURL = iconCandidates.first(where: { FileManager.default.fileExists(atPath: $0.path) }),
    ...
    fputs("error: StashbarIconDark.png not found\n", stderr)
    exit(1)
}
...
NSGradient(colors: [
    color(0.058, 0.070, 0.082),
    color(0.110, 0.136, 0.158)
...
for index in 0..<6 {
    ...
    fillRounded(
        ...
        color: index == 3 ? color(0.420, 0.561, 0.651, 0.18) : NSColor.white.withAlphaComponent(0.045)
    )
...
drawShadowed(
    ...
    fill: color(0.925, 0.927, 0.918, 0.94),
...
let menuLabelAttributes: [NSAttributedString.Key: Any] = [
    ...
    .foregroundColor: color(0.050, 0.068, 0.090)
]
...
for (index, pill) in menuPills.enumerated() {
    ...
    let attributes: [NSAttributedString.Key: Any] = [
        ...
        .foregroundColor: isActive ? color(0.965, 0.961, 0.945) : color(0.185, 0.220, 0.248)
    ]
    ...
    drawCapsule(rect, fill: isActive ? color(0.420, 0.561, 0.651) : NSColor.black.withAlphaComponent(0.08))
...
let brandAttributes: [NSAttributedString.Key: Any] = [
    ...
    .foregroundColor: color(0.900, 0.920, 0.930)
]
...
for (index, chip) in chips.enumerated() {
    let foreground = index == 0 ? color(0.965, 0.961, 0.945) : color(0.800, 0.850, 0.880)
    let fill = index == 0 ? color(0.420, 0.561, 0.651) : NSColor.white.withAlphaComponent(0.08)
    let attributes: [NSAttributedString.Key: Any] = [
...
drawShadowed(
    stagePath,
    fill: color(0.118, 0.148, 0.174, 0.97),
    shadowColor: NSColor.black.withAlphaComponent(0.46),
    blur: 42,
    offset: CGSize(width: 0, height: -24)
)

let previewFrame = stageRect.insetBy(dx: 74, dy: 88)
fillRounded(previewFrame, radius: 44, color: color(0.085, 0.110, 0.132))
...
drawShadowed(
    ...
    fill: color(0.900, 0.920, 0.930, 0.96),
    shadowColor: NSColor.black.withAlphaComponent(0.30),
    blur: 24,
