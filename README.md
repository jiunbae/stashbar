...
- 여러 감시 폴더를 등록하고 폴더별로 최신 파일을 정렬해서 표시
- 아이콘/목록/계층 보기 모드를 전환하며 파일을 탐색
- 슬라이더로 미리보기 크기를 즉시 조절하고 썸네일을 빠르게 로드
- Finder 스타일의 다중 선택, `⌘C/⌘X/⌘V`, Delete, Space(Quick Look) 단축키 지원
